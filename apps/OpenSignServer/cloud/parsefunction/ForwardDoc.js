...
import { assertDocumentMailAllowed, callerIdentity, makeMailQuota, recipientList } from './mailGuard.js';

async function loadForwardDocument(docId) {
  ...
  return docRes ? docRes.toJSON() : null;
}

export function makeForwardDoc({
  send = sendMailWithAttachment,
  loadDocument = loadForwardDocument,
  consumeQuota = makeMailQuota(),
} = {}) {
  return async function forwardDoc(request) {
    try {
      if (!request.user) {
      ...
      if (docId && isReceipents) {
        const _docRes = await loadDocument(docId);
        if (!_docRes) {
          throw new Parse.Error(Parse.Error.OBJECT_NOT_FOUND, 'Document not found.');
        }
        if (!request.master) {
          // The creator or a signer may forward the signed copy, and only to the
          // document's own signers or owner, within the daily limit.
          const caller = callerIdentity(request.user);
          const to = recipientList(recipients);
          assertDocumentMailAllowed(_docRes, caller, to);
          await consumeQuota(caller.id, to.length);
        }
      ...
      console.log('Err in forwardDoc', err?.message);
      throw err;
    }
  };
}

export default makeForwardDoc();
